...
| `ADMIN_CAPACITY_SECRET` | Shared secret for `/api/admin/capacity-debug` and `/api/admin/capacity-reset`. |
...
## Capacity debug helpers

* `GET /api/admin/capacity-debug?secret=ADMIN_CAPACITY_SECRET` returns the cached snapshot of recent day counts (optionally filter with `&date=YYYY-MM-DD`).
* `POST /api/admin/capacity-reset?secret=ADMIN_CAPACITY_SECRET` with `{ "date": "YYYY-MM-DD" }` clears the cached entry for that local date so you can force a fresh snapshot on the next availability/webhook run.
